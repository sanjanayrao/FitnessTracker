     ...
     async updateDB(){
      ...
      async getFoodDB(){
        let data = await fetch('https://mysqlcs639.cs.wisc.edu/foods', {
          ...
          let currFood =  await fetch('https://mysqlcs639.cs.wisc.edu/meals/' + this.props.navigation.state.params["id"] + '/foods' , {
            method: 'GET',
            headers: {
                'Accept' : 'application/json',
                'Content-Type' : 'application/json',
            ...
            currFood = await currFood.json();
            let currF =[];
            for (curr of currFood["foods"]){
              curr["new"] = false;
              currF.push(curr);
            }
            this.setState({foods: currF});
      }
      returnToStart(){
      ...
      async removeFood(item){
        ...
        let data =  await fetch('https://mysqlcs639.cs.wisc.edu/meals/' + this.props.navigation.state.params["id"] + '/foods/' + item["id"]  , {
            method: 'DELETE',
            headers: {
                'Accept' : 'application/json',
                'Content-Type' : 'application/json',
