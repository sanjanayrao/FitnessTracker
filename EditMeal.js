...
export default class EditMeal extends React.Component{
    constructor(props) {
     ...
     async updateDB(){
       ...
       let data = await fetch('https://mysqlcs639.cs.wisc.edu/meals/' + this.state.id, {
            method: 'PUT',
            headers: {
    ...
    async componentDidMount(){
     ...
     this.setState({name: data["name"]});
      ...
      editMeal(){
          if(this.state.name){
          this.updateDB().then(()=>{ 
            this.props.navigation.navigate('EditFoods', {token: this.props.navigation.state.params["token"], id: this.state.id});
